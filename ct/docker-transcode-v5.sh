...
export PCT_OPTIONS="
  ...
  $PW
"
bash -c "$(wget -qLO - https://raw.githubusercontent.com/holl595/Proxmox/main/ct/create_lxc.sh)" || exit
if [ "$CT_TYPE" == "0" ]; then
...
lxc-attach -n $CTID -- bash -c "$(wget -qLO - https://raw.githubusercontent.com/holl595/Proxmox/main/install/$var_install.sh)" || exit
