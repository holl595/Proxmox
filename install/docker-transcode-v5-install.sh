...
$STD apt install --assume-yes docker.io
$STD sudo apt-get -y remove apparmor
...
read -r -p "Would you like to add Portainer Agent? <y/N> " prompt
if [[ $prompt == "y" || $prompt == "Y" || $prompt == "yes" || $prompt == "Yes" ]]; then
  PORTAINER_AGENT="Y"
else
  PORTAINER_AGENT="N"
